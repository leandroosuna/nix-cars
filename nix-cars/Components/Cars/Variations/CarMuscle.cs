...
{
    public class CarMuscle : Car
    {
        public override void LoadModel()
        {
            base.LoadModel();
            (id, model) = CarManager.GetModel("muscle");
            colors = [new Vector3(0,1f,.2f), Vector3.One, Vector3.One];
        }
            ...
            foreach (var mesh in model.Meshes)
            {
                ...
                foreach (var part in mesh.MeshParts)
                {
                    if (m == 0)
                    {
                        ...
                        else if (mp == 2) // exhaust
                        {
                            e.SetColor(Color.Gray.ToVector3());
                            //e.SetKD(0f);
                        }
                        else if (mp == 3) //licence
                        {
                        ...
                        else if (mp == 5) // front lights
                        {
                            e.SetColor(Color.White.ToVector3());
                            e.SetKD(0f);
                        }
                        else if (mp == 6) //back lights
                        {
