...
namespace nix_cars.Components.Collisions
{
    ...
    public class OrientedBoundingBox
    {
        ...
        public Vector3 ToOBBSpace(Vector3 point)
        {
            ...
            var inverseOrientation = Matrix.Invert(Orientation);
            // Transform the point into OBB space
            Vector3 difference = point - Center;
            return Vector3.Transform(difference, inverseOrientation);
        }
        ...
        public bool Intersects(CollisionTriangle triangle)
        {
            // Transform triangle vertices from world space to OBB-Space
            Vector3[] obbSpaceVerts = new Vector3[3];
            for (int i = 0; i < 3; i++)
            {
                obbSpaceVerts[i] = ToOBBSpace(triangle.v[i]);
            }

            // Create the local AABB (OBB is axis-aligned in its own space)
            BoundingBox localAABB = new BoundingBox(-Extents, Extents);

            // Check intersection using existing AABB-Triangle method
            return TriangleAABBIntersection(obbSpaceVerts, localAABB);
        }
        private bool TriangleAABBIntersection(Vector3[] verts, BoundingBox aabb)
        {
            // Implementation of Tomas Akenine-Möller's AABB-Triangle intersection
            // This is the optimized version from "Real-Time Collision Detection"
            float triangleMin, triangleMax;
            float boxMin, boxMax;

            // Test box normals (AABB face normals)
            for (int i = 0; i < 3; i++)
            {
                Project(verts, Vector3.UnitX, out triangleMin, out triangleMax);
                if (triangleMax < aabb.Min.X || triangleMin > aabb.Max.X) return false;

                Project(verts, Vector3.UnitY, out triangleMin, out triangleMax);
                if (triangleMax < aabb.Min.Y || triangleMin > aabb.Max.Y) return false;

                Project(verts, Vector3.UnitZ, out triangleMin, out triangleMax);
                if (triangleMax < aabb.Min.Z || triangleMin > aabb.Max.Z) return false;
            }
            ...
            float offset = Vector3.Dot(normal, verts[0]);
            Project(aabb, normal, out boxMin, out boxMax);
            if (boxMax < offset || boxMin > offset) return false;
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    Vector3 cross = Vector3.Cross(aabbEdges[i], edge1);
                    Project(aabb, cross, out boxMin, out boxMax);
                    Project(verts, cross, out triangleMin, out triangleMax);
                    if (boxMax < triangleMin || boxMin > triangleMax) return false;

                    cross = Vector3.Cross(aabbEdges[i], edge2);
                    Project(aabb, cross, out boxMin, out boxMax);
                    Project(verts, cross, out triangleMin, out triangleMax);
                    if (boxMax < triangleMin || boxMin > triangleMax) return false;
                }
        ...
        private void Project(Vector3[] points, Vector3 axis, out float min, out float max)
        {
            min = float.MaxValue;
            max = float.MinValue;
            foreach (Vector3 p in points)
            {
                float val = Vector3.Dot(p, axis);
                min = Math.Min(min, val);
                max = Math.Max(max, val);
            }
        }

        private void Project(BoundingBox box, Vector3 axis, out float min, out float max)
        {
